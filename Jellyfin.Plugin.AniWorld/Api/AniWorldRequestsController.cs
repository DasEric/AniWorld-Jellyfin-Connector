...
/// <summary>Jellyfin API für Suche, Benutzeranfragen und Admin-Entscheidungen.</summary>
[ApiController]
[Route("AniWorldRequests")]
[Authorize]
[Produces(MediaTypeNames.Application.Json)]
public sealed class AniWorldRequestsController : ControllerBase
{
    ...
    private readonly AniWorldRequestApplicationService _application;

    public AniWorldRequestsController(
        AniWorldClient aniWorld,
        MediaAccessGrantStore grants,
        UserRateLimiter rateLimiter,
        SecretStore secrets,
        AniWorldRequestApplicationService application)
    {
        _aniWorld = aniWorld;
        _grants = grants;
    ...
    public IActionResult GetStatus()
    {
        ...
        return Ok(new
        {
            configured = !string.IsNullOrWhiteSpace(config?.AniWorldUrl) && _secrets.HasApiKey,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!TryReadAniWorldImageUrl(url, out var normalized))
        {
        ...
        try
        {
            var image = await _aniWorld.GetImageAsync(normalized, cancellationToken).ConfigureAwait(false);
    ...
    public Task<IActionResult> GetSeries([Required] string url, CancellationToken cancellationToken)
        => ProxyGranted(url, token => _aniWorld.GetSeriesAsync(url, token), cancellationToken);

    [HttpGet("Seasons")]
    public Task<IActionResult> GetSeasons([Required] string url, CancellationToken cancellationToken)
        => ProxyGranted(url, token => _aniWorld.GetSeasonsAsync(url, token), cancellationToken);

    [HttpGet("Episodes")]
    public Task<IActionResult> GetEpisodes([Required] string url, CancellationToken cancellationToken)
        => ProxyGranted(url, token => _aniWorld.GetEpisodesAsync(url, token), cancellationToken);

    [HttpGet("Providers")]
    public Task<IActionResult> GetProviders([Required] string url, CancellationToken cancellationToken)
        => ProxyGranted(url, token => _aniWorld.GetProvidersAsync(url, token), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return BadRequest(new { error = "Diese AniWorld-URL wurde nicht durch deine aktuelle Suche freigegeben." });
        }
    ...
    /// <summary>
    /// Liest Discover-Einträge aus AniWorld-Endpunkten.
    /// AniWorld liefert entweder ein Array direkt oder { items: [...] }.
    /// </summary>
        ...
        IReadOnlyDictionary<string, string> allowedSources)
    {
        ...
        foreach (var item in items.EnumerateArray())
        {
            ...
            if (!TryReadAniWorldImageUrl(posterUrl, out _))
            {
    ...
    /// <summary>
    /// Liest und validiert eine AniWorld-Bild-Proxy-URL.
    /// AniWorld nutzt /api/proxy-image?url=... statt /api/img?url=...
    /// </summary>
    private static bool TryReadAniWorldImageUrl(string value, out string normalized)
    {
        ...
        // AniWorld-Bild-URLs beginnen mit /api/proxy-image?url=
        if (!value.StartsWith("/api/proxy-image?", StringComparison.Ordinal))
        {
            return false;
        }
    ...
    private IActionResult AniWorldError(AniWorldException exception)
    {
    ...
    private IActionResult ApplicationError(Exception exception)
        => exception switch
        {
            AniWorldException aniWorldException => AniWorldError(aniWorldException),
            AniWorldApplicationException applicationException => StatusCode(
                NormalizeStatus(applicationException.StatusCode),
                new { error = applicationException.Message }),
            _ => StatusCode(StatusCodes.Status502BadGateway, new { error = "AniWorld konnte die Anfrage nicht verarbeiten." }),
        };
