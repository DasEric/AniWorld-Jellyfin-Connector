...
public sealed class Plugin : BasePlugin<PluginConfiguration>, IHasWebPages
{
    ...
    private const string PluginDisplayName = "AniWorld Requests";
    private const string MenuLinkUrl = "#/aniworld-requests";
    ...
    public override string Description => "AniWorld-Suche und Download-Anfragen für alle Jellyfin-Benutzer.";
    ...
    private void UpdateMenuLink(bool enabled)
    {
        ...
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException or ArgumentException or NotSupportedException or Newtonsoft.Json.JsonException)
        {
            ...
            Console.Error.WriteLine("[AniWorld Requests] Could not update jellyfin-web/config.json: " + exception.Message);
        }
    }
    ...
    private void UpdateIndexHtml(bool inject)
    {
        try
        {
            ...
            {
                const string script = "<script plugin=\"AniWorld Requests\" src=\"../AniWorldRequests/InjectionScript\" defer></script>";
    ...
    {
        if (string.IsNullOrWhiteSpace(configuration.AniWorldApiKey))
        {
            return false;
        }

        Secrets.SetApiKey(configuration.AniWorldApiKey);
        configuration.AniWorldApiKey = string.Empty;
        return true;
    }
    ...
    public static IReadOnlyList<PluginPageInfo> CreatePages()
    {
        ...
        [
            new PluginPageInfo
            {
                Name = "AniWorldRequestsConfig",
                EmbeddedResourcePath = $"{root}.Web.config.html",
                EnableInMainMenu = true,
                MenuSection = "server",
                MenuIcon = "settings",
                DisplayName = "AniWorld Requests Settings",
            },
            new PluginPageInfo
            {
                Name = "AniWorldRequestsConfigJS",
                EmbeddedResourcePath = $"{root}.Web.config.js",
            },
            new PluginPageInfo
            {
                Name = "AniWorldRequests",
                EmbeddedResourcePath = $"{root}.Web.requests.html",
                DisplayName = PluginDisplayName,
            },
            new PluginPageInfo
            {
                Name = "AniWorldRequestsJS",
