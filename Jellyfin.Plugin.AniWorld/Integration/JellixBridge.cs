...
namespace Jellyfin.Plugin.AniWorld.Integration;
...
public sealed class JellixBridge
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (operation == "status")
        {
            ...
            var status = await _aniWorld.CheckHealthAsync(cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            id = request.Id,
            title = SafeText(request.Title, 300, "AniWorld"),
        ...
        CancellationToken cancellationToken)
    {
        ...
        var items = requests.Take(500).Select(request => new JellixRequestResponse(
            request.Id,
            SafeText(request.Title, 300, "AniWorld"),
            MapStatus(request),
    ...
    private static AniWorldApplicationException Rejected(string message)
