...
/// <summary>
/// Gemeinsame Request-Applikationsschicht für die Jellyfin HTTP-API und
/// die optionale In-Process Jellix-Bridge.
/// </summary>
public sealed class AniWorldRequestApplicationService
{
    ...
    internal AniWorldRequestApplicationService(
        AniWorldClient aniWorld,
        RequestStore store,
        MediaAccessGrantStore grants,
        UserRateLimiter rateLimiter,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new AniWorldApplicationException(HttpStatusCode.BadRequest, "Der Suchbegriff ist ungültig.");
        }

        if (issueSelectionTokens && mediaType is null)
        {
            throw new AniWorldApplicationException(HttpStatusCode.BadRequest, "Der Medientyp muss Film oder Serie sein.");
        }
        ...
        if (sources.Count == 0)
        {
            throw new AniWorldApplicationException(
                ...
                "Die ausgewählte AniWorld-Quelle ist für diesen Medientyp nicht freigegeben oder deaktiviert.");
        }
        ...
        var groups = await Task.WhenAll(sources.Select(async item =>
        {
            ...
            {
                return new AniWorldSearchGroup(item.Id, item.Label, null, exception.Message);
            }
        })).ConfigureAwait(false);

        if (!issueSelectionTokens)
        {
            return new AniWorldSearchResponse(groups, []);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new AniWorldApplicationException(HttpStatusCode.BadRequest, validationError);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var queueIds = requests
            ...
            .Select(item => item.AniWorldQueueId!.Value)
            .Distinct()
            .Take(500)
            .ToArray();
        ...
        // AniWorld liefert die gesamte Queue – wir filtern client-seitig nach bekannten IDs.
        ...
        {
            if (request.AniWorldQueueId.HasValue
                && byQueueId.TryGetValue(request.AniWorldQueueId.Value, out var state))
            {
    ...
    public async Task<IReadOnlyList<AniWorldProgressInfo>> GetProgressAsync(
        string userId,
        CancellationToken cancellationToken)
    {
        if (!Allow(userId, "progress", 30))
        ...
        bool refreshAvailability = false)
    {
        ...
        try
        {
            ...
            var queueResult = await _aniWorld.QueueAsync(request, cancellationToken).ConfigureAwait(false);
            var warning = queueResult.AcceptedEpisodeCount.HasValue
                && queueResult.AcceptedEpisodeCount.Value != request.Episodes.Count
                ? $"AniWorld hat {queueResult.AcceptedEpisodeCount.Value} von {request.Episodes.Count} geplanten Episoden bestätigt. Die Warteschlange wurde nicht erneut gesendet."
                : null;
            await _store.MarkQueuedAsync(id, queueResult.QueueId, decidedBy, warning, CancellationToken.None).ConfigureAwait(false);
        }
        catch (Exception exception) when (exception is AniWorldException or AniWorldApplicationException or HttpRequestException or OperationCanceledException)
        {
            var error = exception switch
            {
                AniWorldException aniWorldException => aniWorldException.Message,
                AniWorldApplicationException applicationException => applicationException.Message,
                _ => "Die Übergabe an AniWorld wurde unterbrochen.",
            };
        ...
        bool requireGrant)
    {
        ...
        var detailTask = _aniWorld.GetSeriesAsync(request.SeriesUrl, cancellationToken);
        var seasonsTask = _aniWorld.GetSeasonsAsync(request.SeriesUrl, cancellationToken);
        ...
        {
            throw new AniWorldApplicationException(HttpStatusCode.BadGateway, "AniWorld hat keine gültige Staffelliste geliefert.");
        }

        if (seasons.GetArrayLength() > 100)
        {
            throw new AniWorldApplicationException(HttpStatusCode.BadRequest, "Der Titel enthält mehr als 100 Staffeln und kann nicht sicher automatisch geplant werden.");
        }
        ...
        foreach (var season in seasonItems)
        {
            ...
            foreach (var episode in seasonEpisodes)
            {
                ...
                {
                    throw new AniWorldApplicationException(HttpStatusCode.BadGateway, "AniWorld hat eine Episode ohne gültige Staffel- oder Folgennummer geliefert. Es wurde nichts eingereiht.");
                }
                ...
                if (missing.Count > MaxEpisodesPerRequest)
                {
                    throw new AniWorldApplicationException(
                        HttpStatusCode.BadRequest,
                        $"Es fehlen mehr als {MaxEpisodesPerRequest} Episoden. Bitte den Titel in mehreren Schritten einreihen.");
                }
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var response = await _aniWorld.GetProvidersAsync(episodeUrl, cancellationToken).ConfigureAwait(false);
        ...
        catch (AniWorldException)
        {
            throw new AniWorldApplicationException(
                ...
                "AniWorld konnte die verfügbaren Download-Provider nicht ermitteln.");
        }
    }
    ...
    private static AniWorldApplicationException NoProviderAvailable()
        => new(
            HttpStatusCode.BadGateway,
            "Für die fehlenden Inhalte ist aktuell kein Download-Provider verfügbar.");

    private static AniWorldApplicationException DownloadOptionsChanged()
        => new(
            HttpStatusCode.BadRequest,
            "Die gewählte Sprache oder der Provider ist nicht mehr verfügbar. Bitte die Auswahl neu öffnen.");

    /// <summary>
    /// Liest den Queue-Fortschritt aus der AniWorld-Queue-Antwort.
    /// AniWorld liefert die gesamte Queue; hier wird nach den bekannten IDs gefiltert.
    /// </summary>
    internal static IReadOnlyList<AniWorldProgressInfo> ReadProgress(
        JsonElement response,
        IReadOnlyCollection<long> requestedIds)
    {
        var output = new List<AniWorldProgressInfo>();
        var allowed = requestedIds.ToHashSet();
        var seen = new HashSet<long>();

        // AniWorld /api/queue liefert ein Array direkt oder { items: [...] }
        JsonElement items;
        ...
        foreach (var item in items.EnumerateArray())
        {
            ...
            var phase = item.TryGetProperty("phase", out var phaseValue)
                ...
                && phaseValue.GetString() is "download" or "ffmpeg"
                    ...
                    : "download";
            output.Add(new AniWorldProgressInfo(queueId, status!, current, total, percent, phase));
        }
    ...
    private static AniWorldApplicationException TooManyRequests()
        => new(HttpStatusCode.TooManyRequests, "Zu viele Anfragen. Bitte kurz warten.");

    private static AniWorldApplicationException SourceNotAllowed()
        => new(HttpStatusCode.BadRequest, "Die angegebene AniWorld-Quelle ist nicht freigegeben oder deaktiviert.");

    private static AniWorldApplicationException InvalidSeasonList()
        => new(HttpStatusCode.BadGateway, "AniWorld hat eine unvollständige oder ungültige Staffelliste geliefert. Es wurde nichts eingereiht.");
...
{
    public AniWorldApplicationException(HttpStatusCode statusCode, string message)
        : base(message)
    {
...
public sealed record AniWorldSourceInfo(
    [property: JsonPropertyName("id")] string Id,
    [property: JsonPropertyName("label")] string Label,
    [property: JsonPropertyName("media_types")] IReadOnlyList<string> MediaTypes);

public sealed record AniWorldSearchGroup(
    [property: JsonPropertyName("source")] string Source,
    [property: JsonPropertyName("label")] string Label,
    [property: JsonPropertyName("data")] JsonElement? Data,
    [property: JsonPropertyName("error")] string? Error);

public sealed record AniWorldSearchResponse(
    IReadOnlyList<AniWorldSearchGroup> Groups,
...
public sealed record AniWorldProgressInfo(
