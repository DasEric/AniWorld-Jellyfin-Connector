(function () {
  'use strict';
  const RUNTIME_KEY = '__aniWorldRequestsInjection';
  if (window[RUNTIME_KEY]) return;
  window[RUNTIME_KEY] = true;
  const MENU_ID = 'aniworld-requests-sidebar';
  const MODERN_MENU_ID = 'aniworld-requests-modern-sidebar';
  const MODAL_ID = 'aniworld-requests-modal';
  const OFFICIAL_LINK_HASH = '#/aniworld-requests';
  ...
  function inject() {
    ...
    if (sidebar && !document.getElementById(MENU_ID)) {
      ...
      entry.setAttribute('is', 'emby-linkbutton'); entry.setAttribute('data-itemid', 'aniworld-requests');
  ...
  function closeRequests(restoreFocus) {
    ...
    if (activeDispose) {
      ...
      activeDispose = null;
      try { dispose(); } catch (error) { console.warn('[AniWorld Requests] Cleanup failed:', error); }
    } else {
  ...
  async function open() {
    ...
    if (existing) {
      const existingClose = existing.querySelector('[data-aniworld-close]');
    ...
    overlay.setAttribute('aria-label', 'AniWorld Anfragen');
    ...
    overlay.innerHTML = '<div style="position:sticky;top:0;z-index:5;display:flex;justify-content:flex-end;padding:.5rem;background:#111;border-bottom:1px solid #333"><button type="button" data-aniworld-close aria-label="AniWorld Anfragen schließen" style="min-width:2.75rem;min-height:2.75rem;border:0;background:transparent;color:#fff;font-size:2rem;line-height:1;cursor:pointer">×</button></div><div data-content><div style="padding:3rem;text-align:center">Laden…</div></div>';
    const closeButton = overlay.querySelector('[data-aniworld-close]');
    ...
    try {
      const html = await client.fetch({ url: client.getUrl('AniWorldRequests/Page'), type: 'GET', dataType: 'text' });
      ...
      const module = await import(client.getUrl('AniWorldRequests/PageScript') + '?v=' + Date.now());
      if (generation !== openGeneration || !overlay.isConnected) return;
      if (module.default) {
      ...
      if (generation === openGeneration && overlay.isConnected) content.textContent = 'AniWorld Requests konnte nicht geladen werden.';
    }
  }
  function start() {
